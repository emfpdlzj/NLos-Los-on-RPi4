# sa_train_export_fp32_tflite.py
import os
...
# --- Feature Extraction (Dense(30)→BN→Dense(16,ReLU))를 Conv1D(커널=1)로 구현
...
# 5) 2단계: Feature Extractor 동결 후 미세조정
...
plt.tight_layout(); plt.savefig("result/sa_confmat.png", dpi=150)

# 7) Keras 모델 저장(선택)
model.save("6.selfatt.h5")

# 8) **Unquantized FP32 TFLite** 내보내기 (라즈베리파이 tflite-runtime용)
converter = tf.lite.TFLiteConverter.from_keras_model(model)
# 최적화/양자화 옵션 지정하지 않으면 FP32 그대로 변환됨
tflite_fp32 = converter.convert()
with open("sa_fp32.tflite", "wb") as f:
    f.write(tflite_fp32)

print("Saved: sa_fp32.tflite (I/O=float32, input shape (1,100))")
