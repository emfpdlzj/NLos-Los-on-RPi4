...
from keras.layers import Conv1D, MaxPooling1D, GlobalMaxPooling1D, Dense, Dropout, LSTM, Bidirectional
...
model.add(MaxPooling1D(2))
model.add(LSTM(32, return_sequences=True))  # 첫 번째 LSTM
model.add(LSTM(32))                         # 두 번째 LSTM
# Stacked LSTM: LSTM 레이어를 여러 개 겹쳐 쌓은 구조
...
model.fit(
    ...
    callbacks=[CSVLogger('result/log_modelname.csv')]
)
...
model.save("2.cnn_stackedlstm.h5");